...
/**
 * This is the model class for table "{{%buy_classes}}".
 *
 * @property int $id
 * @property int $buy_id 购买者id
 * @property int $buy_studio 购买者画室
 * @property int $classes_id 云课件id
 * @property int $classes_studio 云课件画室
 ...
 */
class BuyClasses extends ActiveRecord
{
    public $code;
    /**
     * @inheritdoc
    ...
    public function beforeSave($insert)
    {
        if (parent::beforeSave($insert)) {
            if ($this->isNewRecord) {
                $this->classes_studio = Campus::findOne(Classes::findOne($this->classes_id)->campus_id)->studio_id;
            }else{
    ...
    {
        return [
            [['buy_id', 'buy_studio', 'classes_id', 'classes_studio', 'created_at', 'updated_at', 'active_at', 'status'], 'integer'],
            ['role','in','range'=>[10,20]],
            ['code','string','length' => [8, 11]],
            [['buy_id','classes_id','role','price','buy_studio'],'required'],
            [['price'], 'number'],
        ];
    }
    ...
    public static function getBuyStatus($user_id,$role,$class_id) {
        $model = self::find()
                      ->where(['buy_id'=>$user_id,'role'=>$role,'classes_id'=>$class_id,'status'=>10])
    ...
    {
        return $this->hasOne(Classes::className(), ['id' => 'classes_id'])->alias('classes');
    }
}
