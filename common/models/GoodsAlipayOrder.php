 ...
 */
class GoodsAlipayOrder extends \yii\db\ActiveRecord
{
    /**
     * @inheritdoc
     */
    ...
    public function rules()
    {
        return [
            ...
            [['user_id', 'payment_type', 'quantity', 'status','goods_id'], 'integer'],
            [['notify_time', 'gmt_create', 'gmt_payment','total_fee', 'price', 'gmt_refund', 'create_time'], 'safe'],
            [[ 'discount'], 'number'],
            [['body'], 'string'],
            [['notify_type', 'notify_id', 'sign_type', 'out_trade_no', 'trade_no'], 'string', 'max' => 64],
            [['sign'], 'string', 'max' => 3000],
            [['subject', 'seller_email', 'buyer_email'], 'string', 'max' => 128],
            [['trade_status', 'seller_id', 'buyer_id', 'is_total_fee_adjust', 'use_coupon', 'refund_status','user_type','goods_type'], 'string', 'max' => 32],
