...
class UserHomework extends ActiveRecord
{
        ...
        if (parent::beforeSave($insert)) {
            ...
            if ($this->isNewRecord) {
                if($this->created_time){
                     $this->created_at = strtotime($this->created_time);
                }
            }else{
                //数据更新
    ...
    public function rules()
    {
        return [
            ...
            [['comments','score','video','created_time'], 'string'],
           # [['score'], 'number'],
            ['image', 'image', 
    ...
    public static function getMonth($int = 0) {
        $firstday = date("Y-m-01",strtotime("+$int month"));

        $end = strtotime("$firstday +1 month -1 seconds");
