...
class CourseMaterial extends ActiveRecord
{
    ...
    public function fields() {
        ...
        if(self::$is_show){
            $fields['picture'] = function () {
                return $this->getMaterials(Picture::className(), 'picture', 'image', Group::TYPE_PICTURE);
            };  
            $fields['video'] = function () {
                return $this->getMaterials(Video::className(), 'video', 'preview', Group::TYPE_VIDEO);
            };     
        }        
        unset(
            $fields['id'],
    ...
    //获取课件教案
    public static function getCourseList($admin_id,$page=0,$limit=10) {

       $admin_ids  =  array_column(Admin::getVisua($admin_id),'admin_id');

       return self::find()
                    ->where(['admin_id'=>$admin_ids,'status'=>self::STATUS_ACTIVE])
                    ->offset($page*$limit)
                    ->limit($limit)
