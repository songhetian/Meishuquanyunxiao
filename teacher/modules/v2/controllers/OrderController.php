...
*/
	...
	{
		public function actionTextClasses(){
			...
			$order = GoodsAlipayOrder::findOne(2029);
		...
		public function actionTestsell(){
			$sell = new InviteSell();
			$sell->user_id = 123;
			$sell->user_type = 'teacher';
			$sell->buy_type = "alipay";
			$sell->goods_type = "vip";
			$sell->order_id = 123;
			$sell->status = 0;
			$sell->time = date("Y-m-d H:i:s",time());
			$sell->name = "会员一个月";
			$sell->price = 1;
			$sell->save();
		}
	    ...
	    public function actionOrderInfo(){
	    	if(!empty(Yii::$app->request->get('id')) && !empty(Yii::$app->request->get('user_id')) && !empty(Yii::$app->request->get('user_type')) && !empty(Yii::$app->request->get('type'))){
	    		$vip_price = UserVipPrice::findOne(Yii::$app->request->get('id'));
	    		
				$price = $vip_price->price;
				...
				$bizcontent = json_encode([
		            'body'=>$vip_price->name,
		            'subject'=>$vip_price->name,  
		            'out_trade_no'=> $out_trade_no,//此订单号为商户唯一订单号  
		            'total_amount'=> number_format($price,2) ,//保留两位小数
		            'product_code'=>'QUICK_MSECURITY_PAY'
		        ]);
				$request->setNotifyUrl("https://api.teacher.meishuquanyunxiao.com/v2/order/order-check");
				$request->setBizContent($bizcontent);
				...
				$order->out_trade_no = $out_trade_no;
				$order->price = number_format($price,2);
				$order->subject = $vip_price->name;
				$order->vip_id = $vip_price->id;
				if($order->save()){
					$data = array("success"=>true,"data"=>$response,"message"=>"获得成功");
	        		return $data;
				}
		...
		public function actionOrderCheck(){
			...
			if($_POST['trade_status'] == 'TRADE_SUCCESS'){
				$order = AlipayOrder::findOne(['out_trade_no' => $_POST['out_trade_no'],'status'=>2]);
				if(!empty($order)){
	                $order->load(Yii::$app->getRequest()->getBodyParams(), '');
	                $vip_price = UserVipPrice::findOne($order->vip_id);
					if($order->user_type == "teacher"){
						$temp = 1;
						$user = Admin::findOne(['id'=>$order->user_id]);
					}else if($order->user_type == "student"){
						$temp = 2;
						$user = User::findOne(['id'=>$order->user_id]);
					}else if($order->user_type == "family"){
						$temp = 999;
						$user = Family::findOne(['id'=>$order->user_id]);
					}
					$order->status = 1;
	            	if($order->save()){

	            		if(!empty($user->vip_time) && strtotime($user->vip_time) > time()){
							$time = $user->vip_time;
						}else{
						...
						if(!empty($code)){
							...
							$code->due_time = date('Y-m-d',strtotime('+'.$vip_price->membership_type.'months',strtotime($time)));
							$code->save();
							
						}
						$user->vip_time = date('Y-m-d',strtotime('+'.$vip_price->membership_type.'months',strtotime($time)));
						$user->save();

						$sell = new InviteSell();
						$sell->user_id = $order->user_id;
						$sell->user_type = $order->user_type;
						$sell->buy_type = "alipay";
						$sell->goods_type = "vip";
						$sell->order_id = $order->id;
						$sell->status = 0;
						$sell->time = date("Y-m-d H:i:s",time());
						$sell->name = $order->body;
						$sell->price = (int)($order->price*100);
						$sell->save();
	...
	public function actionWxorderInfo(){
		if(!empty(Yii::$app->request->get('id')) && !empty(Yii::$app->request->get('user_id')) && !empty(Yii::$app->request->get('user_type')) && !empty(Yii::$app->request->get('type'))){
			$vip_price = UserVipPrice::findOne(Yii::$app->request->get('id'));
			if(Yii::$app->request->get('type') == "ios"){
				$price = $vip_price->price_ios;
			}else{
				$price = $vip_price->price;
			}
			...
			if (Yii::$app->request->get('user_id') == 4276 && Yii::$app->request->get('user_type')== 'teacher') {
				$price = 0.01;
			}else if(Yii::$app->request->get('user_id') == 2442 && Yii::$app->request->get('user_type')== 'family'){
				$price = 0.01;
			}else if(Yii::$app->request->get('user_id') == 2008 && Yii::$app->request->get('user_type')== 'family'){
				$price = 0.01;
			}else if(Yii::$app->request->get('user_id') == 4025 && Yii::$app->request->get('user_type')== 'teacher'){
				$price = 0.01;
			}

			$out_trade_no = "WXVIP".Yii::$app->request->get('user_id').Yii::$app->request->get('user_type').time().$vip_price->membership_type.mt_rand(100, 999);
			return  $this->weChatPay($out_trade_no,$price,$vip_price->name,Yii::$app->request->get('user_id'),Yii::$app->request->get('user_type'),Yii::$app->request->get('id'));
		}
	}
	public function weChatPay($order_num,$price,$name,$user_id,$user_type,$id){
		$json = array();
		//生成预支付交易单的必选参数:
		$newPara = array();
		//应用ID
		...
		$newPara["notify_url"] = "http://api.teacher.meishuquanyunxiao.com/v2/order/wxorder-check";
		//交易类型
	...
	public function actionWxorderCheck(){
		...
		if($value_array['return_code'] == "SUCCESS" && $value_array['result_code'] == "SUCCESS"){
			$order = WechatOrder::findOne(['out_trade_no' => $value_array['out_trade_no']]);
			//未付款状态的订单
			if(!empty($order) && $order->status == 2){
				$vip_price = UserVipPrice::findOne($order->product_id);
				if($order->user_type == "teacher"){
					$temp = 1;
					$user = Admin::findOne(['id'=>$order->user_id]);
				}else if($order->user_type == "student"){
            	...
            	$order->save();
					...
					$sell = new InviteSell();
						$sell->user_id = $order->user_id;
						$sell->user_type = $order->user_type;
						$sell->buy_type = "wechat";
						$sell->goods_type = "vip";
						$sell->order_id = $order->id;
						$sell->status = 0;
						$sell->time = date("Y-m-d H:i:s",time());
						$sell->name = $order->body;
						$sell->price = $order->total_fee;
						$sell->save();
			}
		}
	...
	*/
	...
	public function actionIosIapPay() {
        ...
        try
        {
        	...
        	if(!is_array($info) && $info == 21007){
                $info = $this->getReceiptData($receipt,true);
              }
            // 通过product_id 来判断是下载哪个资源
            // switch($info['product_id']){
            //     case 'com.meishuquanyunxiao.artworld1':
                     $status['status'] = 1;
                    //Header("Location:xxxx.zip");
            //         break;
            //     case 'com.meishuquanyunxiao.artworld3':
            //         $status['status'] = 1;
            //         //Header("Location:xxxx.zip");
            //         break;
            //     case 'com.meishuquanyunxiao.artworld6':
            //         $status['status'] = 1;
            //         //Header("Location:xxxx.zip");
            //         break;
            //     case 'com.meishuquanyunxiao.artworld12':
            //         $status['status'] = 1;
            //         //Header("Location:xxxx.zip");
            //         break;
            // }
            ...
            if ($status['status']==1) {
            	$order = IapOrder::findOne(['transaction_id' => $info['transaction_id']]);
            	if(empty($order)){

            		$vip_price = UserVipPrice::findOne(['product_id'=>$info['product_id']]);
					if($_POST['user_role'] == "teacher"){
						$user = Admin::findOne(['id'=>$_POST['user_id']]);
						$temp = 1;
					}else if($_POST['user_role'] == "student"){
						$user = User::findOne(['id'=>$_POST['user_id']]);
						$temp = 2;
					}else if($_POST['user_role'] == "family"){
						$user = Family::findOne(['id'=>$_POST['user_id']]);
						$temp = 999;
					}
						if(!empty($user->vip_time) && strtotime($user->vip_time) > time()){
							$time = $user->vip_time;
					...
					$iap_order = new IapOrder;
					$iap_order->user_id = $_POST['user_id'];
					$iap_order->user_type = $_POST['user_role'];
					$iap_order->membership_type = $vip_price->membership_type;
			...
			if($_POST['user_role'] == "teacher"){
				$status['data'] = Admin::findOne($_POST['user_id']);
			}else if($_POST['user_role'] == "student"){
				$status['data'] =  SimpleUser::find()
				...
				$status['data'] = Family::findOne($_POST['user_id']);
			}
			 $file  = './iaplog.txt';
			 ob_start();
			 var_dump($status);
    ...
    public function getReceiptData($receipt, $isSandbox = false){
        ...
        if ($errno != 0) {
            throw new NotFoundHttpException($errmsg, $errno);
        }

        $data = json_decode($response);
        if (!is_object($data)) {
            throw new NotFoundHttpException('无效的响应数据');
        }
        ...
        if (!isset($data->status) || $data->status != 0) {
            ...
            throw new NotFoundHttpException('无效的收据:'.$data->status);
        }
        if(empty($data->latest_receipt_info)){
          $max = $this->ReceiptSort($data->receipt->in_app);
        }else{
          $in_app = $this->ReceiptSort($data->receipt->in_app);
          $latest = $this->ReceiptSort($data->latest_receipt_info);
          $max = ($in_app->purchase_date_ms > $latest->purchase_date_ms) ? $in_app : $latest;
        }
        $info =  array(
          ...
          'app_item_id' => 0,
          'original_transaction_id' => $max->original_transaction_id,
          'web_order_line_item_id'  =>  $max->web_order_line_item_id,
        );
        $info['is_trial_period'] = ($max->is_trial_period == true) ? 1 : 2;

        return $info;
    }
    public function ReceiptSort($array){
        $count = count($array);  
        if($count <= 0)  
        {  
            return false;  
        ...
        {  
            for($k=$count-1; $k>$i; $k--)  
            {  
                if($array[$k]->purchase_date_ms < $array[$k-1]->purchase_date_ms)  
                {  
                    $tmp = $array[$k];  
                    $array[$k] = $array[$k-1];  
                    $array[$k-1] = $tmp;  
